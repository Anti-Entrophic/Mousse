...
def init_optimizer(
    ...
    if hp.optimizer == "dion":
        ...
        opt = Mousse(
            ...
            shampoo_beta=float(hp.shampoo_beta),
            LR_correction=hp.LR_correction,
...
def main():
    ...
    def get_lr_cosine(it):
        ...
        decay_ratio = (it - warmup_iters) / (hp.num_iterations - warmup_iters)
    ...
    for step in range(start_step, hp.num_iterations + 1):
        ...
        approx_time = 1000 * (time.time() - t0)
        if MASTER_PROCESS and not cli_args.no_wandb and not cli_args.debug:
            log_data = {
                "train/loss": train_loss.item(),
                "train/grad_norm": grad_norm.item(),
                "time/training_time_ms_per_step": approx_time,
            }
            if opt_logs:
