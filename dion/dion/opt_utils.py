...
def pad_state_batch(batch: List[Dict], batch_size: int) -> List[Dict]:
    """
    Insert dummy state so the batch has exactly `batch_size` elements.

    The elements of dummy state are None, we need to filter these states later in real computation.
    """
    assert len(batch) > 0
    assert len(batch) <= batch_size
